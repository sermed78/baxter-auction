    ...
    if (items.length === 0) {
        return (
            <div className="text-center py-12 bg-white rounded-lg border border-dashed border-gray-300">
                <p className="text-gray-500">No auctions at the moment.</p>
            </div>
        );
    }
            ...
            {view === 'grid' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sortedItems.map((item) => {
                        ...
                        const winner = isEnded ? item.bids[0]?.user : null;

                        return (
                            <Link href={`/auction/item/${item.id}`} key={item.id} className="group">
                                <article className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border overflow-hidden flex flex-col h-full ${isEnded ? 'border-gray-300 opacity-80' : 'border-gray-200'}`}>
                                    <div className="relative aspect-[4/3] bg-gray-100">
                                        {item.imageUrl ? (
                                            <img
                                                src={item.imageUrl}
                                                alt={item.title}
                                                className={`object-cover w-full h-full ${isEnded ? 'grayscale-[30%]' : ''}`}
                                            />
                                        ) : (
                                            ...
                                            {isEnded ? (
                                                <div className="space-y-1">
                                                    <div className="flex justify-between items-end">
                                                        <div>
                                                            <p className="text-xs text-gray-500 uppercase">Winning Bid</p>
                                                        ...
                                                        {winner && (
                                                            <div className="text-right">
                                                                <p className="text-xs text-gray-500 uppercase">Winner Tag</p>
                                                                <p className="text-sm font-mono font-bold text-[#003D87]">
                                                                    #{winner.tagId}
                                                                </p>
                                                            </div>
                                                        )}
                    ...
                    {sortedItems.map((item) => {
                        ...
                        return (
                            <Link href={`/auction/item/${item.id}`} key={item.id} className="block group">
                                <article className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border overflow-hidden flex flex-row h-32 md:h-40 ${isEnded ? 'border-gray-300 opacity-80' : 'border-gray-200'}`}>
                                    <div className="relative w-32 md:w-48 bg-gray-100 flex-shrink-0">
                                        ...
                                        {isEnded && (
                                            <div className="absolute top-1 right-1 bg-gray-800 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                                                ENDED
                                            </div>
                                        )}
                                        {isEndingSoon && (
                                    ...
                                    <div className="p-4 flex-1 flex flex-col justify-between">
                                        ...
                                        <div className="flex justify-between items-end">
                                            <div>
                                                <p className="text-xs text-gray-500 uppercase">{isEnded ? 'Winning Bid' : 'Current Bid'}</p>
                                                <p className="text-lg font-mono font-medium text-gray-900">
                                                    {currentBid.toLocaleString('sv-SE')} kr
                                                </p>
                                            </div>
                                            {isEnded && winner ? (
                                                <div className="text-right">
                                                    ...
                                                    <p className="text-sm font-mono font-bold text-[#003D87]">#{winner.tagId}</p>
                                                </div>
                                            ) : (
